...
public class NeedRadiantProgress : RimWorld.Need {
    ...
    public NeedRadiantProgress(Pawn pawn) : base(pawn) {
    ...
    private bool IsEligibleForRankup(Pawn pawn, Trait trait) {
        ...
        switch (idealLevel) {
            case 1:
                WhtwlRadiantNeedLevelupChecker.UpdateIsSatisfiedReq1_2(pawnStats);
                eligible = pawnStats.GetRequirements(trait.def, pawnStats.props.req12).isSatisfied;
                break;
            case 2:
                WhtwlRadiantNeedLevelupChecker.UpdateIsSatisfiedReq2_3(pawnStats);
                eligible = pawnStats.GetRequirements(trait.def, pawnStats.props.req23).isSatisfied;
                break;
            case 3:
                WhtwlRadiantNeedLevelupChecker.UpdateIsSatisfiedReq3_4(pawnStats);
...
public static class PatchRadiantProgressNeed {
    public static void Postfix(Pawn_NeedsTracker instance, NeedDef nd, ref bool result, Pawn pawn) {
        ...
        if (nd == CosmereRosharDefs.WhtwlRadiantProgress &&
            pawn.story?.traits?.HasTrait(CosmereRosharDefs.WhtwlRadiantWindrunner) == true) {
            result = true;
        } else if (nd == CosmereRosharDefs.WhtwlRadiantProgress &&
                   pawn.story?.traits?.HasTrait(CosmereRosharDefs.WhtwlRadiantTruthwatcher) == true) {
            result = true;
        } else if (nd == CosmereRosharDefs.WhtwlRadiantProgress &&
                   pawn.story?.traits?.HasTrait(CosmereRosharDefs.WhtwlRadiantEdgedancer) == true) {
            result = true;
        } else if (nd == CosmereRosharDefs.WhtwlRadiantProgress &&
                   pawn.story?.traits?.HasTrait(CosmereRosharDefs.WhtwlRadiantSkybreaker) == true) {
            result = true;
        } else if (nd == CosmereRosharDefs.WhtwlRadiantProgress &&
                   pawn.story?.traits?.HasTrait(CosmereRosharDefs.WhtwlRadiantWindrunner) == false) {
            result = false;
        } else if (nd == CosmereRosharDefs.WhtwlRadiantProgress &&
                   pawn.story?.traits?.HasTrait(CosmereRosharDefs.WhtwlRadiantTruthwatcher) == false) {
            result = false;
        } else if (nd == CosmereRosharDefs.WhtwlRadiantProgress &&
                   pawn.story?.traits?.HasTrait(CosmereRosharDefs.WhtwlRadiantEdgedancer) == false) {
            result = false;
        } else if (nd == CosmereRosharDefs.WhtwlRadiantProgress &&
                   pawn.story?.traits?.HasTrait(CosmereRosharDefs.WhtwlRadiantSkybreaker) == false) {
            result = false;
        }
    }
}
