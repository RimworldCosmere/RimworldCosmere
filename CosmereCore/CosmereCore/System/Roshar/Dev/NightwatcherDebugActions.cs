...
public static class NightwatcherDebugActions {
    ...
    private static void GiveNightwatcherBoon(Pawn pawn) {
        ...
        for (int i = 0; i < boons.Count; i++) {
            ...
            options.Add(
                new DebugMenuOption(
                    $"[T{boon.powerTier}] {boon.LabelCap}",
                    DebugMenuOptionMode.Action,
                    () => {
                        NightwatcherSystem.ApplyBoon(pawn, boon);
                        Messages.Message(
                            $"Applied boon '{boon.LabelCap}' to {pawn.LabelShort}",
            ...
            options.Add(
                new DebugMenuOption(
                    ...
                    () => {
                        ...
                        Messages.Message(
                            $"Applied curse '{curse.LabelCap}' to {pawn.LabelShort}",
