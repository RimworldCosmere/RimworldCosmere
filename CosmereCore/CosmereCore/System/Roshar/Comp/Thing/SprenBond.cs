...
public class SprenBondProperties : CompProperties {
    public SprenBondProperties() {
        compClass = typeof(SprenBond);
    }
}

public class SprenBond : ThingComp {
    ...
    private static TraitDef? traitKind;
    private static TraitDef? traitAbrasive;
    private static TraitDef? traitNerves;
    private static TraitDef? traitNaturalMood;
    private static TraitDef? traitIndustriousness;
    ...
    private static List<TraitDef> GetTraitPool() {
        ...
        traitKind = RimWorld.TraitDefOf.Kind;
        traitAbrasive = RimWorld.TraitDefOf.Abrasive;
        traitNerves = DefDatabase<TraitDef>.GetNamedSilentFail("Nerves");
        traitNaturalMood = DefDatabase<TraitDef>.GetNamedSilentFail("NaturalMood");
        traitIndustriousness = DefDatabase<TraitDef>.GetNamedSilentFail("Industriousness");
    ...
    public float GetRecoveryMultiplier() {
        float multiplier = 1f;
        for (int i = 0; i < personalityTraits.Count; i++) {
            TraitDef trait = personalityTraits[i];
            if (trait == traitNaturalMood) multiplier *= 1.1f;
            else if (trait == traitIndustriousness) multiplier *= 1.1f;
        }
