...
public sealed class AutocastRuleEditorDialog : Verse.Window {
    ...
    public override void DoWindowContents(Rect inRect) {
        ...
        using (new TextBlock(GameFont.Medium, TextAnchor.MiddleLeft, Color.white))
            Widgets.Label(new Rect(inRect.x, y, inRect.width, 30f), "CC_Autocast_Editor_Title".Translate(abilityLabel.Named("ABILITY")));
        y += 34f;
        ...
        Widgets.Label(capLabel, "CC_Autocast_Editor_MaxReserveSpend".Translate(((int)(rule.CostCapFraction * 100f)).Named("PERCENT")));
        ...
        if (trigger.Kind == AutocastTriggerKind.Drafted) {
            ...
            using (new TextBlock(GameFont.Small, TextAnchor.MiddleLeft, Color.white))
                Widgets.Label(
                    valueLabel,
                    isPercent
                        ? "CC_Autocast_Editor_PercentLabel".Translate(((int)(trigger.Threshold * 100f)).Named("PERCENT"))
