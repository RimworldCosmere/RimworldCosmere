...
public static class HemalurgicChargeUtility {
    public static void DriveHemalurgicCharge(
        Pawn donor,
        Pawn surgeon,
    ...
    public static void ApplyConnectionSteal(Pawn donor) {
        ConnectionStealRegistry.NotifyConnectionStolen(donor);
    }

    public static void RemoveAllPowers(Pawn donor) {
        DrainCosmereGenes(donor);
        ClearInvestitureAndTrait(donor);
    }
    ...
    public static List<string> StealAllAbilities(Pawn donor) {
        ...
        DrainCosmereGenes(
            donor,
            onAllomantic: a => stolenDefNames.Add(a.def.defName),
            onFeruchemic: f => stolenDefNames.Add(f.def.defName)
        );
        ClearInvestitureAndTrait(donor);
        return stolenDefNames;
    }

    private static void DrainCosmereGenes(
        Pawn donor,
        Action<Allomancer>? onAllomantic = null,
        Action<Feruchemist>? onFeruchemic = null
    ) {
        List<Allomancer> allomantic = donor.genes.GetAllomanticGenes();
        for (int i = allomantic.Count - 1; i >= 0; i--) {
            onAllomantic?.Invoke(allomantic[i]);
        ...
        for (int i = feruchemic.Count - 1; i >= 0; i--) {
            onFeruchemic?.Invoke(feruchemic[i]);
            donor.genes.RemoveGene(feruchemic[i]);
        }
    }

    private static void ClearInvestitureAndTrait(Pawn donor) {
        InvestitureHolder? holder = donor.TryGetComp<InvestitureHolder>();
        if (holder != null) {
