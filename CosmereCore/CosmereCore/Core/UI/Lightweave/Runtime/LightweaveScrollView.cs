...
public readonly record struct LightweaveScrollView : IDisposable {
    ...
    private static Texture2D? upArrowTex;
    private static Texture2D? downArrowTex;
    ...
    private static Texture2D GetArrowTexture(bool up) {
        if (up) {
            return upArrowTex ??= BuildArrowTexture(true);
        }

        return downArrowTex ??= BuildArrowTexture(false);
    }
