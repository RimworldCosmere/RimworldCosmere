...
/// <list type="bullet">
/// <item>No fade-in delay - the tooltip appears immediately on the frame the pointer enters <paramref name="rect"/>.</item>
/// <item>No <c>TipSignal</c> hash caching - the <paramref name="build"/> closure is invoked fresh each frame.</item>
/// <item>No built-in string translation or formatting - the Lightweave tree is responsible for all text rendering.</item>
/// <item>No integration with vanilla tooltip stacking priority - this overlay always renders on top of the current Lightweave pass.</item>
/// </list>
/// </para>
/// </summary>
public static class AsTooltip
{
    ...
    /// <param name="build">Factory invoked each frame the pointer is over <paramref name="rect"/> to produce the tooltip content node.</param>
