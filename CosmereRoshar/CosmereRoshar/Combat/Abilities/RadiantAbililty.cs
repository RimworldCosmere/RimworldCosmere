...
public class CommandRadiantAbility : Command {
    ...
    private bool AbriasionCheck() {
        if (ability.def == Defs.Cosmere_Roshar_SurgeOfAbrasion) {
    ...
    public override void ProcessInput(Event ev) {
        ...
        bool hasRadiantTrait = pawn.story.traits.HasTrait(Defs.Cosmere_Roshar_Trait_RadiantWindrunner) ||
    ...
    private bool AbilityPlantGrowSurge() {
        if (ability.def == Defs.Cosmere_Roshar_SurgeOfGrowth) {
    ...
    private bool AbilityDivisionSurge() {
        if (ability.def == Defs.Cosmere_Roshar_SurgeOfDivision) {
            TargetingParameters tp = new TargetingParameters {
                canTargetPawns = true,
    ...
    private bool AbilityHealSurge() {
        if (ability.def == Defs.Cosmere_Roshar_SurgeOfHealing) {
            TargetingParameters tp = new TargetingParameters {
                canTargetPawns = true,
                canTargetAnimals = true,
                canTargetItems = false,
    ...
    private bool AbilitySummonShardblade() {
        if (ability.def == Defs.Cosmere_Roshar_SummonShardblade) {
            ability.Activate(pawn, pawn);
            return true;
        }
    ...
    private bool AbilityLashingUpward() {
        if (ability.def == Defs.Cosmere_Roshar_LashingUpward) {
            TargetingParameters tp = new TargetingParameters {
                canTargetPawns = true,
                canTargetAnimals = false,
                canTargetItems = false,
    ...
    private bool AbilityWindRunnerFlight() {
        if (ability.def != Defs.Cosmere_Roshar_WindRunnerFlight) return false;

        float cost = pawn
            .GetAbilityComp<SurgeGravitation>(
                Defs.Cosmere_Roshar_WindRunnerFlight.defName
            )!
    ...
    private void StartCustomTargeting(
        ...
        void MainAction(LocalTargetInfo chosenTarget) {
            Verse.AI.Job job = new CastAbilityOnTarget(
                Defs.Cosmere_Roshar_CastAbilityOnTarget,
                chosenTarget,
                ability
            );
            if (triggerAsJob) {
