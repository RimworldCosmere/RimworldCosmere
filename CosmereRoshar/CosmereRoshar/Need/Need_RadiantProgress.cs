...
public static class Patch_RadiantProgress_Need {
    public static void Postfix(Pawn_NeedsTracker __instance, NeedDef nd, ref bool __result, Pawn ___pawn) {
        ...
        if (nd == CosmereRosharDefs.whtwl_RadiantProgress &&
            ___pawn.story?.traits?.HasTrait(CosmereRosharDefs.whtwl_Radiant_Windrunner) == true) {
            __result = true;
        } else if (nd == CosmereRosharDefs.whtwl_RadiantProgress &&
                   ___pawn.story?.traits?.HasTrait(CosmereRosharDefs.whtwl_Radiant_Truthwatcher) == true) {
            __result = true;
        } else if (nd == CosmereRosharDefs.whtwl_RadiantProgress &&
                   ___pawn.story?.traits?.HasTrait(CosmereRosharDefs.whtwl_Radiant_Edgedancer) == true) {
            __result = true;
        } else if (nd == CosmereRosharDefs.whtwl_RadiantProgress &&
                   ___pawn.story?.traits?.HasTrait(CosmereRosharDefs.whtwl_Radiant_Skybreaker) == true) {
            __result = true;
        } else if (nd == CosmereRosharDefs.whtwl_RadiantProgress &&
                   ___pawn.story?.traits?.HasTrait(CosmereRosharDefs.whtwl_Radiant_Windrunner) == false) {
            __result = false;
        } else if (nd == CosmereRosharDefs.whtwl_RadiantProgress &&
                   ___pawn.story?.traits?.HasTrait(CosmereRosharDefs.whtwl_Radiant_Truthwatcher) == false) {
            __result = false;
        } else if (nd == CosmereRosharDefs.whtwl_RadiantProgress &&
                   ___pawn.story?.traits?.HasTrait(CosmereRosharDefs.whtwl_Radiant_Edgedancer) == false) {
            __result = false;
        } else if (nd == CosmereRosharDefs.whtwl_RadiantProgress &&
                   ___pawn.story?.traits?.HasTrait(CosmereRosharDefs.whtwl_Radiant_Skybreaker) == false) {
            __result = false;
        }
    }
}
