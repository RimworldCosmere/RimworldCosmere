    ...
    Rem? BottomEnd = null)
{
    ...
    public static RadiusSpec All(Rem v) => new RadiusSpec(v, v, v, v);
    public static RadiusSpec Top(Rem v) => new RadiusSpec(TopLeft: v, TopRight: v);
    public static RadiusSpec StartSide(Rem v) => new RadiusSpec(TopStart: v, BottomStart: v);
    ...
    public Vector4 ResolveVector(Direction dir)
    {
        ...
        float br = BottomRight?.ToPixels() ?? (dir == Direction.Ltr ? BottomEnd?.ToPixels() : BottomStart?.ToPixels()) ?? 0f;
        float bl = BottomLeft?.ToPixels() ?? (dir == Direction.Ltr ? BottomStart?.ToPixels() : BottomEnd?.ToPixels()) ?? 0f;
