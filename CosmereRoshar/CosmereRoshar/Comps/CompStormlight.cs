...
public class CompStormlight : ThingComp {
    ...
    // Surges
    private bool m_AbrasionActive;
    public bool m_BreathStormlight;
    ...
    public void RemoveAllStormlight() {
        m_CurrentStormlight = 0f;
    }

    // Called after loading or on spawn
    public override void PostExposeData() {
        base.PostExposeData();
        Scribe_Values.Look(ref m_AbrasionActive, "abrasionActive");
        Scribe_Values.Look(ref m_CurrentStormlight, "currentStormlight");
        Scribe_Values.Look(ref m_BreathStormlight, CosmereRosharDefs.whtwl_BreathStormlight.defName);
    ...
    public override void CompTickInterval(int delta) {
        if (isActivatedOnPawn == false && parent is Pawn _pawn) {
            return;
        }
    ...
    public override string CompInspectStringExtra() {
        ...
        return "Stormlight: " +
               m_CurrentStormlight.ToString("F0") +
               " / " +
    ...
    public string getTimeRemaining() {
        ...
        int hoursLeft = (int)(Stormlight / stormlightPerHour);
        int daysLeft = 0;
        if (hoursLeft > 24) {
    ...
    private void radiantAbsorbStormlight(Pawn pawn) {
        ...
        foreach (Verse.Thing thing in nearbyThings) {
            if (thing.def.defName.StartsWith("whtwl_Sphere_") && !thing.Position.Roofed(thing.Map)) {
                CompStormlight? stormlightComp = thing.TryGetComp<CompStormlight>();
                if (stormlightComp != null) {
                    float drawnLight = stormlightComp.drawStormlight(absorbAmount);
                    RadiantUtility.GiveRadiantXP(pawn, 0.1f);
                    if (infuseStormlight(drawnLight)) return;
                }
            } else if ((thing.def.defName.StartsWith("Cut") || thing.def.defName.StartsWith("Raw")) &&
    ...
    private void handleSurges(Pawn pawn) {
        ...
        if (m_AbrasionActive) {
            if (pawn != null &&
                pawn.health.hediffSet.GetFirstHediffOfDef(CosmereRosharDefs.whtwl_surge_abrasion) == null) {
                pawn.health.AddHediff(CosmereRosharDefs.whtwl_surge_abrasion);
                drainFactor += 500f;
            }
        } else {
            if (pawn != null &&
                pawn.health.hediffSet.GetFirstHediffOfDef(CosmereRosharDefs.whtwl_surge_abrasion) is Hediff hediff) {
...
public class StormlightProperties : CompProperties {
    ...
    public StormlightProperties() {
        compClass = typeof(CompStormlight);
    }
