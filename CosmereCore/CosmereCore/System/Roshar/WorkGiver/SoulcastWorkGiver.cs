...
namespace Cosmere.System.Roshar.WorkGiver;
...
public class SoulcastWorkGiver : WorkGiver_Scanner {
    ...
    public override bool HasJobOnCell(Pawn pawn, IntVec3 c, bool forced = false) {
        ...
        if (!SoulcastOverlay.TryGetData(c, pawn.Map, out _, out _, out _)) return false;
        return FindSoulcastAbility(pawn) != null;
    }

    public override Verse.AI.Job? JobOnCell(Pawn pawn, IntVec3 cell, bool forced = false) {
        if (!SoulcastOverlay.TryGetData(cell, pawn.Map, out _, out _, out _)) return null;
