...
public class SurgesAndOrdersGenerator : BaseGenerator {
    ...
    public override async Task GenerateAsync() {
        ...
        // Generate Bonded Spren ThingDefs (exclude Bondsmith - they bond godspren, not companion spren)
        var bondedSprenDefOutputDir = FileSystem.Path.Combine(rosharXmlDir, "Defs", "Things", "Pawn", "Animal");
        List<RadiantOrderInfo> bondableOrders = [];
        foreach (RadiantOrderInfo order in orders) {
            if (!string.Equals(order.Name, "bondsmith", StringComparison.OrdinalIgnoreCase)) {
