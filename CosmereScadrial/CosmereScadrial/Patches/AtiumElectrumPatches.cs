...
namespace CosmereScadrial.Patches {
    ...
    public static class AtiumElectrumPatches {
        ...
        private static bool Patch(Verb instance, ref bool result) {
            ...
            var casterHasAtium = casterPawn.health?.hediffSet?.HasHediff(HediffDefOf.Cosmere_Hediff_AtiumBuff) ?? false;
            var targetHasAtium = target.health?.hediffSet?.HasHediff(HediffDefOf.Cosmere_Hediff_AtiumBuff) ?? false;
            var targetHasElectrum =
